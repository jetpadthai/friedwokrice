...
class Siteground_SuperCacher_EsiController extends Mage_Core_Controller_Front_Action {
    /**
    ...
    public function getBlockAction() {
        ...
        if( Mage::helper( 'supercacher/esi' )->shouldResponseUseEsi() ) {
            ...
            if( $esiDataHmac !== ( $hmac = $dataHelper->getHmac( $esiDataParamValue ) ) ) {
                ...
                $appShim = Mage::getModel( 'supercacher/shim_mage_core_app' );
                if( $referer = Mage::helper( 'core/http' )->getHttpReferer() ) {
                    $dummyRequest = Mage::helper( 'supercacher/esi' )
                ...
                if( $block ) {
                    ...
                    $staticDbBlocks = Mage::getResourceModel('cms/block_collection')->load()->toOptionArray();
                    if (is_array($staticDbBlocks))
						foreach($staticDbBlocks as $staticBlockInfo)
						{
							$blockDb = Mage::getModel('cms/block')->setStoreId(Mage::app()->getStore()->getId())->load($staticBlockInfo['value']);

							if (!is_object($blockDb) || !$blockDb->identifier)
								continue;

							$blockIdentifier = $blockDb->identifier;

							if (!$block->getChild($blockIdentifier))
							{
								$staticChildBlock = $this->getLayout()->createBlock('cms/block')->setBlockId($blockIdentifier);
								$staticChildBlock->setEsiOptions( false );
								$block->setChild($blockIdentifier,$staticChildBlock);
							}
						}
                    ...
                    if( !is_null( $flushEvents = $esiData->getFlushEvents() ) ) {
                        $resp->setHeader( 'X-SuperCacher-Flush-Events',
                            implode( ',', $flushEvents ) );
                    }
                    if( $esiHelper->getEsiDebugEnabled() ) {
                        $resp->setHeader( 'X-SuperCacher-Block',
        ...
        Mage::register( 'supercacher_nocache_flag', !$cacheFlag, true );
    }
    ...
    protected function _getEsiBlock( $esiData ) {
        $block = null;
        Varien_Profiler::start( 'supercacher::controller::esi::_getEsiBlock' );
        ...
        if( $blockNode instanceof Varien_Simplexml_Element ) {
            $nodesToGenerate = Mage::helper( 'supercacher/data' )
                ->getChildBlockNames( $blockNode );
            Mage::getModel( 'supercacher/shim_mage_core_layout' )
        ...
        Varien_Profiler::stop( 'supercacher::controller::esi::_getEsiBlock' );
        return $block;
    }
