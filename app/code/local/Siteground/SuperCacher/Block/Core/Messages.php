...
class Siteground_SuperCacher_Block_Core_Messages extends Mage_Core_Block_Messages {
    /**
    ...
    protected function _handleDirectCall( $methodCalled ) {
        ...
        if( $this->_fixMessages() ) {
            ...
            $layout->generateXml();
            $layoutShim = Mage::getSingleton( 'supercacher/shim_mage_core_layout' );
    ...
    protected function _hasInjectOptions() {
        return $this->getEsiOptions() &&
            Mage::helper( 'supercacher/esi' )->shouldResponseUseEsi();
    }
    ...
    protected function _saveMessages( $messages ) {
        if( $this->_fixMessages() && !$this->_isEsiRequest() ) {
            Mage::getSingleton( 'supercacher/session' )
    ...
    /**
     * Load messages saved to supercacher/session
     *
     * @return null
     */
    protected function _loadSavedMessages() {
        $session = Mage::getSingleton( 'supercacher/session' );
    ...
    /**
     * Clear messages saved to supercacher/session
     *
     * @return null
     */
    ...
    /**
     * Check if we should fix the messages behavior to work with supercacher,
     * disable-able for compatibility with other extensions
     *
     * @return bool
     */
    protected function _fixMessages() {
        return Mage::helper( 'supercacher/esi' )->shouldFixFlashMessages();
    }
    ...
    protected function _isEsiRequest() {
        return is_subclass_of( Mage::app()->getRequest(),
            'Siteground_SuperCacher_Model_Dummy_Request' );
    }
}
