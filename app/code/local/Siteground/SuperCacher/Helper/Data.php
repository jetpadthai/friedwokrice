...
class Siteground_SuperCacher_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    /**
     * Get the SuperCacher version
     *
     * @return string
     */
    public function getVersion() {
        return Mage::getConfig()
            ->getModuleConfig( 'Siteground_SuperCacher' )->version;
    }
    ...
    public function freeze( $data ) {
        Varien_Profiler::start( 'supercacher::helper::data::freeze' );
        $frozenData = $this->urlBase64Encode(
            $this->_getCrypt()->encrypt(
                gzdeflate(
                    serialize( $data ),
                    self::COMPRESSION_LEVEL ) ) );
        Varien_Profiler::stop( 'supercacher::helper::data::freeze' );
        return $frozenData;
    }
    ...
    public function thaw( $data ) {
        Varien_Profiler::start( 'supercacher::helper::data::thaw' );
        $thawedData = unserialize(
            gzinflate(
                $this->_getCrypt()->decrypt(
                    $this->urlBase64Decode( $data ) ) ) );
        Varien_Profiler::stop( 'supercacher::helper::data::thaw' );
        return $thawedData;
    }
    ...
    /**
     * Check config to see if SuperCacher should handle the flash messages
     *
     * @return bool
     */
    public function useFlashMessagesFix() {
        return (bool)Mage::getStoreConfig(
            'supercacher_varnish/general/ajax_messages' );
    }

    /**
     * Check config to see if SuperCacher should apply the product list toolbar
     * fix
     *
     * @return bool
     */
    public function useProductListToolbarFix() {
        return (bool)Mage::getStoreConfig(
            'supercacher_varnish/general/fix_product_toolbar' );
    }
    ...
    protected function _getChildBlockNames( $blockNode ) {
        Varien_Profiler::start( 'supercacher::helper::data::_getChildBlockNames' );
        ...
        Varien_Profiler::stop( 'supercacher::helper::data::_getChildBlockNames' );
        return $blockNames;
    }
