...
class Siteground_SuperCacher_Helper_Cron extends Mage_Core_Helper_Abstract {
     ...
     * @var string
     */
    const CRAWLER_URLS_CACHE_ID = 'supercacher_crawler_url_queue';
    ...
    public function getCrawlerClient() {
        if( is_null( $this->_crawlerClient ) ) {
            $this->_crawlerClient = new Varien_Http_Client( null, array(
                'useragent'     => sprintf(
                    'Siteground_SuperCacher/%s Magento/%s Varien_Http_Client',
                    Mage::helper( 'supercacher/data' )->getVersion(),
                    Mage::getVersion() ),
                'keepalive'     => true,
            ) );
    ...
    public function getCrawlerEnabled() {
        return Mage::getStoreConfig( 'supercacher_varnish/general/crawler_enable' );
    }
